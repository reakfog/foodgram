...
from api.utils import (calculate_ingredients, list_contains_unique_objects,
...
class SubscribersSerializer(serializers.ModelSerializer):
    ...
    def get_recipes(self, obj):
        '''
        Method field allows to set length of subset
        with query pamaneter through nested serializer
        '''
        request = self.context.get('request')
...
class SimpleFollowSerializer(serializers.ModelSerializer):
    '''
    Nested Serrializer that allows to unfollow user
    with DELETE through  ManageFollowingsViewSet
    '''
    class Meta:
        model = Following
        fields = (
...
class FollowingBaseSerializer(serializers.ModelSerializer):
    '''
    Serrializer that allows to follow user
    with GET through ManageFollowingsViewSet
    '''
        ...
        if request.method == 'GET':
            if request.user == attrs['author']:
                raise serializers.ValidationError(
                    self.UNABLE_TO_SIGN_FOR_YOURSELF
                )
            if Following.objects.filter(
                user=request.user,
                author=attrs['author'],
            ).exists():
                raise serializers.ValidationError(self.ALREADY_SUBSCRIBED)
        return attrs
...
class IngredientRecipeSerializer(serializers.ModelSerializer):
    '''
    Nested serializer for RecipeReadSerializer
    that allows to configure ingredient fields
    '''
...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if request.method == 'POST' and Recipe.objects.filter(
            ...
            raise serializers.ValidationError(
                self.RECIPE_UNIQUE_CONSTRAINT_MESSAGE
            )

        if request.method == 'DELETE' and request.user != data[
            'recipe'
        ].author:
            raise serializers.ValidationError(
                self.ONLY_AUTHOR_CAN_DELETE_RECIPE
            )
        return data
    ...
    def validate_tags(self, data):
        ...
        if self.context["request"].method == 'PUT':
            ...
            if Recipe.objects.filter(
                ...
                raise serializers.ValidationError(
                    self.TAG_ID_ALREADY_ASSIGNED.format(item_id, recipe_id)
                )
        return data
        ...
        if self.context["request"].method in ['POST', 'PUT']:
            ...
            for element in data:
                ...
                try:
                    ...
                    raise serializers.ValidationError(
                        self.NO_INGREDIENT_IN_DATABASE.format(item_id)
                    )
        return data
...
class ManageFavoriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if request.method == 'GET' and Favorite.objects.filter(
            ...
            raise serializers.ValidationError(
                self.RECIPE_IS_ALREADY_IN_FAVORITES
            )
        return data
...
class ManageCartSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        request = self.context['request']
        if request.method == 'GET' and Cart.objects.filter(
            user=request.user,
            recipe__id=data['recipe'].id
        ).exists():
            raise serializers.ValidationError(
                self.RECIPE_IS_ALREADY_IN_THE_SHOPPING_CART
            )
        return data
